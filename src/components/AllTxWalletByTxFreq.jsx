const AllTxWalletByTxFreq = ({ defaultTx, address }) => {



    const fromAddresses = defaultTx.filter((tx) => (tx.to === address.toLowerCase())).map((tx) => tx.from)
    const toAddresses = defaultTx.filter((tx) => (tx.from === address.toLowerCase())).map((tx) => tx.to)
...
export default AllTxWalletByTxFreq